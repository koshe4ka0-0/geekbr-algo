#Написать программу, которая будет складывать, вычитать, умножать или делить два числа. Числа и знак операции вводятся пользователем. 
#После выполнения вычисления программа не должна завершаться, а должна запрашивать новые данные для вычислений. Завершение программы 
#должно выполняться при вводе символа '0' в качестве знака операции. Если пользователь вводит неверный знак (не '0', '+', '-', '*', '/'),
# то программа должна сообщать ему об ошибке и снова запрашивать знак операции. Также сообщать пользователю о невозможности деления на ноль,
#  если он ввел 0 в качестве делителя.

while True:

    answer = input("Введите операцию (+, -, *, /) или знак выхода из программы (0): ")

    if answer == '0':
        break

    elif answer != '0' and answer != '+' and answer != '-' and answer != '*' and answer != '/':
        print("Ошибка ввода знака операции")

    else: 
        num1 = int(input("Введите первое число: "))
        num2 = int(input("Введите второе число: "))

        if answer == '+':
            print(f' {num1} + {num2} = {num1 + num2}')

        elif answer == '-':
            print(f' {num1} - {num2} = {num1 - num2}')

        elif answer == '*': 
            print(f' {num1} * {num2} = {num1 * num2}')

        elif answer == '/':
            if num2 == 0:
                print("Ошибка. Деление на ноль невозможно")

            else:
                print(f' {num1} / {num2} = {num1 / num2}')
